...
export default function App() {
  ...
  const [visibleFontIds, setVisibleFontIds] = useState<Set<string>>(
    () => new Set(presetFonts.map((font) => font.id)),
  )
  const [statusMessage, setStatusMessage] = useState('')
  const uploadedFontsRef = useRef<FontOption[]>([])

  const visibleFonts = fonts.filter((font) =>
    visibleFontIds.has(font.id),
  )

  useEffect(() => {
    uploadedFontsRef.current = fonts.filter(
      (font) => font.source === 'upload',
    )
  }, [fonts])
  ...
  const showFontGroup = (fontNames: Set<string>) => {
    setVisibleFontIds(
      new Set(
        fonts
          .filter((font) => fontNames.has(font.name))
          .map((font) => font.id),
      ),
    )
  }
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    const results = await Promise.allSettled(
      Array.from(files).map(loadUploadedFont),
    )
    ...
    const loadedFonts = results
      .filter(
        (
          result,
        ): result is PromiseFulfilledResult<FontOption> =>
    ...
    const failedMessages = results
      ...
      .map((result) =>
        result.reason instanceof Error
          ...
          : 'Unable to import one font file.',
      )
    ...
    setStatusMessage(
      [
        loadedFonts.length
          ? `${loadedFonts.length} font${
              loadedFonts.length === 1 ? '' : 's'
            } imported.`
          : '',
        ...failedMessages,
      ]
  ...
  const handleExport = async () => {
    try {
      ...
      setStatusMessage(
        'PNG exported with visible font rows.',
      )
    } catch (error) {
      setStatusMessage(
        error instanceof Error
          ? error.message
          : 'Unable to export the visible fonts.',
      )
    }
  }
        ...
        <div className="hero-brand">
          ...
          <div>
            ...
            <p className="hero-copy">
              Find your perfect font and export what you like
              for easy sharing!
            </p>
          </div>
        </div>
